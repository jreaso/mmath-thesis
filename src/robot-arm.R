...
set.seed(6)
xD <- maximin_lhd_2d(32, perturb=F)*2*pi
nD <- nrow(xD)
...
BL_em <- simple_BL_emulator(xD=xD, D=fD, xP=xP, theta=1.6, sig=1, mu=0)
...
filled.contour(x=theta_seq, y=theta_seq, z=var_mat, color.palette=var_cols, levels=seq(0, 0.1, 0.01),
               xlab=expression(theta[2]), ylab=expression(theta[4]),
               plot.axes={
                 axis(1, at=tick_vals, labels=tick_labs, las=1)
                 axis(2, at=tick_vals, labels=tick_labs, las=1)
